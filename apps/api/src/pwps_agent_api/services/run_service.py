...
class RunService:
    ...
    async def _create_guided_run(self, request: CreateRunRequest) -> CreateRunResponse:
        result = await start_guided_draft(
            request.input,
        ...
        record = RunRecord(
            ...
            raw_input=result.state.raw_input,
            workflow_state_json=result.state.model_dump(mode="json"),
            outputs_json={},
            trace_json=[event.model_dump(mode="json") for event in result.state.trace],
            pending_decision_json=None
            if result.pending_decision is None
            else result.pending_decision.model_dump(mode="json"),
        )
        self.session.add(record)
        await self.session.commit()
        if self.event_store is not None:
            await self.event_store.publish_many(record.run_id, result.state.trace)
        return CreateRunResponse(run_id=record.run_id, status=result.state.status)
    ...
    async def submit_decision(
        ...
        result = await resume_guided_draft(
            run_id=run_id,
        ...
        record.workflow_state_json = result.state.model_dump(mode="json")
        record.trace_json = [event.model_dump(mode="json") for event in result.state.trace]
        ...
        if result.output_paths:
            record.outputs_json = _read_outputs(result.output_paths)
        await self.session.commit()
        if self.event_store is not None:
            await self.event_store.publish_many(
                record.run_id,
                result.state.trace[previous_trace_length:],
            )
